...
本目录保留三类规范来源，分层目录是唯一权威正文位置：

- `基础/`：跨技术栈、跨任务类型都适用的工程治理、编码、质量、安全与交付规则。
- `专项/`：按技术栈或业务边界适用的后端、前端、接口与 UI 规则。
- `模板/`：用于记录设计、审查、验证和交付证据的可复用模板，不直接产生新的规范规则。

根目录仅保留本 README，不再维护与分层正文重复的平铺副本。

## 快速导航

| 文档 | 用途 |
| --- | --- |
| [工程治理与交付规范](./基础/工程治理与交付规范.md) | 从需求接收到交付的阶段、证据和责任边界 |
| [通用编码与文档规范](./基础/通用编码与文档规范.md) | 文本、代码、配置、文档、路径和依赖的通用约束 |
| [测试与质量门禁规范](./基础/测试与质量门禁规范.md) | 测试设计、检查命令、规范指纹、证据和失败处理 |
| [安全可靠性与发布规范](./基础/安全可靠性与发布规范.md) | 密钥、权限、敏感数据、外部依赖、迁移、观测和回滚 |
| [代码审查与交付规范](./基础/代码审查与交付规范.md) | Review、提交、变更说明、交接和交付结论 |
| [Spring Boot 后端开发规范](./专项/后端/SpringBoot后端开发规范.md) | 后端分层、契约、校验、事务、数据、异步和兼容性 |
| [Vue 3 前端开发规范](./专项/前端/Vue3前端开发规范.md) | 组件、状态、路由、API、交互、安全、可访问性和性能 |
| [接口与数据契约规范](./专项/接口与数据契约规范.md) | 对外接口、数据演进、错误语义、兼容性和文档门禁 |
| [Vue 前端 UI 设计规范](./专项/前端/Vue前端UI设计规范.md) | 管理后台布局、视觉层级、反馈、响应式和动效 |
...
- **基础规则**适用于所有代码、配置、测试、文档和 Agent 交接制品。
...
- 新增规则必须先判断归属：跨技术栈规则放入 `基础/`，技术栈或边界规则放入 `专项/`，记录型内容放入 `模板/`；不得在根目录新增平铺正文。

## 修订记录

| 修订 | 日期 | 内容 |
| --- | --- | --- |
| R4 | 2026-08-06 | 固化“基础 / 专项 / 模板”三层设计，移除平铺副本并补充分类导航。 |
| R3 | 2026-08-06 | 增加“设计自上而下、实现自下而上”的工程实施规范，并要求分层最小验证。 |
